...
async def query(idd: UUID4, db_session: DatabaseDependency) -> CategoriaOut:
    categoria: CategoriaOut = await db_session.execute(select(CategoriaModel).filter_by(id=idd)).scalars().first()

    if not categoria:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, 
            detail=f"Categoria não encontrada pelo id = {idd}"
            )
